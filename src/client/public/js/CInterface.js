function CInterface(iPlayerTeam, iOpponentTeam) {
    ...
    this._init = function () {
        ...
        _pStartPosExit = {x: CANVAS_WIDTH - (oSprite.height / 2) - 90, y: (oSprite.height / 2) + 15};
        _oButExit = new CGfxButton(_pStartPosExit.x, _pStartPosExit.y, oSprite, s_oStage);
        _oButExit.addEventListener(ON_MOUSE_UP, this._onExit, this);

        var oSprite = s_oSpriteLibrary.getSprite('but_restart_small');
        _pStartPosRestart = {x: _pStartPosExit.x - oSprite.height - 100, y: _pStartPosExit.y};
        _oButRestart = new CGfxButton(_pStartPosRestart.x, _pStartPosRestart.y, oSprite, s_oStage);
        _oButRestart.addEventListener(ON_MOUSE_UP, this._onRestart, this);

        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            var oSprite = s_oSpriteLibrary.getSprite('audio_icon');
            _pStartPosAudio = {x: _pStartPosRestart.x - oSprite.height - 118, y: _pStartPosExit.y};
    ...
    this.createButStart = function () {
        ...
        var oSpriteButStart = s_oSpriteLibrary.getSprite("but_go");
