function CMenu() {
    ...
    this._init = function () {
        ...
        s_oStage.addChild(_oLogo);
         ...
         _oPlayText = new createjs.Text("Click play to start", "30px " + FONT_GAME, "#fff");
        _oPlayText.x = CANVAS_WIDTH/2;
        _oPlayText.y = CANVAS_HEIGHT/2 + 100;
        _oPlayText.textBaseline = "alphabetic";
        _oPlayText.textAlign = "center";
        s_oStage.addChild(_oPlayText);
        ...
        // Leaderboard button (using the same sprite as info button)
    //     _pStartPosLeaderboard = {x: _pStartPosInfo.x + oSpriteInfo.width / 2 + 15, y: (oSpriteInfo.height / 2) + 15};
    //     _oButLeaderboard = new CGfxButton(_pStartPosLeaderboard.x, _pStartPosLeaderboard.y, oSpriteInfo, s_oStage);
    //     _oButLeaderboard.addEventListener(ON_MOUSE_UP, this._onButLeaderboardRelease, this);
        
    //   //  if(_oButInfo.visible && _oButLeaderboard.visible){
    //         _oButLeaderboard.visible = false;
    //   //  }
    ...
    this._onButLeaderboardRelease = function () {
        var oLeaderboardPanel = new CLeaderboardPanel(s_oSpriteLibrary.getSprite("msg_box"));
        oLeaderboardPanel.show();
    };
